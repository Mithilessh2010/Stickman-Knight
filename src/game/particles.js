...
function addParticle(world, particle, priority = false) {
  if (!world?.particles) return;
  if (world.particles.length >= MAX_PARTICLES) {
    if (!priority) return;
    world.particles.splice(0, world.particles.length - MAX_PARTICLES + 1);
  }
...
function addEffect(world, effect) {
  ...
  if (world.effects.length >= MAX_EFFECTS) world.effects.splice(0, world.effects.length - MAX_EFFECTS + 1);
...
export function spawnRing(world, x, y, color, radius) {
  addEffect(world, { kind: 'ring', x, y, r: 4, maxR: radius, color, life: 22, maxLife: 22 });
}

export function spawnSlash(world, x, y, angle, length, color) {
  addEffect(world, { kind: 'slash', x, y, angle, length, life: 10, maxLife: 10, color });
}
